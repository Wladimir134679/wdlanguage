...
* **Обращение одно на все типы**: `x.имя` ≡ `x["имя"]`, различие только в
  `AccessStyle` для форматтера. Ключ толкует контейнер: у массива и строки число —
  позиция (отрицательное считается от конца), диапазон — срез (`a[1..3]`, границы
  включительны), у объекта и то и другое остаётся ключом. Числовой ключ нормализуется
  одним местом — `runtime/Indexes`, и дисциплины там две: адресация строга
  (`a[i]`, `insert`, `remove`), отрезок подрезается (`a[from..to]`, `slice`).
  См. `docs/access.md` и `Interpreter.visitAccess`.
