...
* **Поток заводится только через `CallContext.threads()`** (`value/ScriptThreads`),
  ...
  он не выполняет, умеет только `interrupt`, а попади он в реестр — считался бы в квоту
  и прерывал сам себя. Пул (`th.pool`) берёт фабрику потоков у того же реестра
  (`ScriptThreads.reserve`) и занимает квоту целиком, пока не закрыт. **Поток, который
  прерыванием не разбудить** (стоит в `accept`, `read`, `readLine`), обязан объявить
  будильник третьим аргументом `start(имя, тело, будильник)`: без него закрытие ждёт
  его весь свой срок и называет в логе. Закрытие запуска — строго в порядке `stopScriptThreads(ms)` →
...
* **Новый лимит**: поле в `runtime/Limits` (ноль — «без предела») → проверка
  ...
  в `wdl-cli/Main` → тест в `LimitsTest` → строка в `docs/limits.md`. Выключенный путь
  не должен дорожать: сначала `limits.counting()`, потом всё остальное.
