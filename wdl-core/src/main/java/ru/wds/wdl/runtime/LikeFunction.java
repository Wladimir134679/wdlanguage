...
final class LikeFunction implements FunctionValue {
    ...
    /**
     * Аннотации тоже целевые, и без этого обёртка была бы полуправдой: декоратор
     * пишут ровно затем, чтобы регистратор нашёл цель по её данным, а обёртка,
     * потерявшая их, отдала бы роутеру пустой объект.
     */
    @Override
