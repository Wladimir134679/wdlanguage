package ru.wds.wdl.runtime;

import ru.wds.wdl.debug.Debugger;
import ru.wds.wdl.metrics.Metrics;
import ru.wds.wdl.profile.Profiler;
import ru.wds.wdl.module.ModuleSource;
import ru.wds.wdl.module.ModuleUnits;
import ru.wds.wdl.module.NativeModules;
import ru.wds.wdl.resolve.Linker;
import ru.wds.wdl.runtime.members.BuiltinMembers;
import ru.wds.wdl.runtime.members.MemberTable;
import ru.wds.wdl.source.Span;

import java.util.Objects;
import java.util.Set;
import java.util.concurrent.ConcurrentHashMap;
import java.util.concurrent.atomic.AtomicLong;
import java.util.concurrent.atomic.LongAdder;

/**
 * Один запуск скрипта: всё, что принадлежит ему целиком, а не области видимости
 * и не файлу.
 * <p>
 * Раньше эти три вещи лежали полями в {@link ExecutionContext} и переносились из копии
 * в копию поодиночке. Разница не в удобстве: контекст — это <b>место</b> в выполнении
 * (какие имена видно, какой файл, какой кадр), а запуск — это <b>сеанс</b>, и живёт он
 * дольше любого места в нём. Пока их держали вместе, функция, вытащенная наружу и
 * вызванная приложением, оставалась без сеанса: {@code ExecutionContext.call} доставал
 * его у вызывающего, а у вызывающего из чужого потока его не было.
 * <p>
 * Теперь сеанс носит с собой сама функция ({@link UserFunction}) и сам класс
 * ({@link WdlClass}). Отсюда главное свойство языка на стороне встраивания:
 * <b>значение-функцию можно передать куда угодно и вызвать откуда угодно</b> — она
 * принесёт с собой свои модули, свои формы классов и свои классы ошибок. От вызывающего
 * ей нужен только вывод: печатать надо туда, где позвали.
 *
 * <h2>Что здесь мутабельно и почему</h2>
 * {@link Modules} был изменяемым всегда — «модуль выполняется один раз за запуск»
 * иначе не выразить. {@link #modules} вдобавок ещё и заменяем, но только до первой
 * инструкции скрипта: {@code withModules} и {@code withNativeModules} собирают запуск,
 * а не меняют его на ходу. Замена внутри одного {@code Run}, а не создание нового,
 * нужна ровно затем, чтобы у запуска был один сеанс от начала до конца: классы прелюдии
 * создаются раньше, чем приложение успевает выдать источник модулей, и достанься им
 * другой сеанс — {@code report()} у ошибки выполнялся бы не в том запуске, где ошибка
 * случилась.
 *
 * <h2>Потоки: замка больше нет</h2>
 * Раньше вход в скрипт снаружи брал один замок на весь сеанс, и внутри запуска
 * одновременно работал ровно один поток. Это был GIL, и держал он не удобство,
 * а корректность: области видимости, поля объектов и реестр модулей были обычными
 * неконкурентными структурами. Теперь они конкурентные — и замок снят.
 * <p>
 * <b>Что запуск обещает автору скрипта</b> (полностью — в {@code docs/threads.md}):
 * одно обращение — чтение или запись переменной, поля, элемента — атомарно и видимо
 * всем потокам. Выражение целиком — нет: {@code count = count + 1} из двух потоков
 * теряет обновления, и склеивают его {@code synchronized} на функции, {@code th.lock()}
 * и {@code th.counter()}.
 * <p>
 * <b>Режима «вернуть старый замок» здесь нет, и это решение.</b> Такой был — один
 * {@code if} на входе, — и держался он на рассуждении «чужие скрипты писались
 * в расчёте на очередь». Оказалось, что он несовместим с {@code sys.thread},
 * появившимся в том же выпуске: главный поток входит в скрипт и держит замок,
 * порождённый поток встаёт на нём, {@code t.join()} ждёт вечно — простейший
 * {@code spawn} + {@code join} вешался намертво. Совместимость с невыпущенной версией
 * не стоит мины под самой заметной возможностью языка, поэтому семантика здесь одна.
 *
 * <h2>Что теперь считается по потоку</h2>
 * Счётчик внешних входов — свойство потока, а не сеанса: цепочка «скрипт → приложение →
 * скрипт» у каждого потока своя, и складывать их в одно число значило бы, что восемь
 * рабочих потоков упираются в предел на четвёртом обороте каждый.
 *
 * <h2>Лимиты выполнения</h2>
 * {@link Limits} — то, чем запуск отвечает на «а если скрипт чужой»: шаги, время
 * и квота потоков. Проверка одна на всё и живёт в {@link #checkpoint(Span)}; зовут её
 * оттуда же, откуда раньше спрашивали флаг прерывания, — из циклов интерпретатора,
 * из тела функции и из создания экземпляра. Точек ровно столько, потому что
 * зациклиться, не пройдя ни разу ни через итерацию, ни через вызов, нельзя,
 * а считать узлы дерева значило бы платить в самой горячей точке за точность,
 * которой никто не пользуется.
 * <p>
 * <b>Время сторожит отдельный поток.</b> Мягкая проверка дедлайна в {@code checkpoint}
 * не достаёт того, кто ушёл в одну долгую операцию на Java: {@code th.sleep(600000)},
 * чтение из сокета, ожидание замка не проходят ни одной точки. Поэтому по истечении
 * срока сторож ({@link #startWatchdog}) прерывает потоки, которые сейчас внутри
 * запуска, — прерывание блокирующие библиотеки уже понимают и отвечают на него
 * остановкой выполнения.
 * Непрерываемый счёт (сортировка миллиона элементов) этим не останавливается ничем:
 * {@code Thread.stop} из JDK убран, и это записано честно — в {@code docs/limits.md}.
 */
public final class Run {

    /** Индексы в массиве состояния потока: {@link #state}. */
    private static final int ENTRIES = 0;
    private static final int STEPS = 1;

    /**
     * Реестр модулей запуска: где их искать и какие уже выполнены.
     * <p>
     * Не {@code final} — единственное поле сеанса, которое приложение задаёт после
     * создания. Почему замена, а не новый {@code Run}, разобрано в javadoc класса.
     * {@code volatile}, потому что задаётся при сборке в одном потоке, а читается
     * из всех.
     */
    private volatile Modules modules;

    /**
     * Куда сообщается время стадий этого запуска.
     * <p>
     * Свойство запуска, а не области видимости, по той же причине, что и модули:
     * «сколько заняло выполнение» — вопрос про сеанс целиком. По умолчанию метрик нет
     * и стоят они ноль ({@link Metrics#off()}); приёмник задаёт хозяин запуска
     * до первой инструкции скрипта — отсюда и {@code volatile}, как у {@link #modules}.
     */
    private volatile Metrics metrics = Metrics.off();

    /**
     * Куда сообщается о вызовах этого запуска.
     * <p>
     * Свойство запуска ровно по той же причине, что метрики и пределы: «какая функция
     * самая горячая» — вопрос про сеанс целиком, а не про область видимости. По умолчанию
     * профиля нет и стоит он ноль ({@link Profiler#off()}); приёмник задаёт хозяин
     * запуска до первой инструкции скрипта — отсюда и {@code volatile}.
     */
    private volatile Profiler profiler = Profiler.off();

    /**
     * Пределы этого запуска.
     * <p>
     * По умолчанию их нет ({@link Limits#none()}) — движок не платит за то, о чём его
     * не просили. Задаются при сборке, до первой инструкции скрипта: менять предел
     * посреди работы значило бы, что «сколько осталось» зависит от момента вопроса.
     */
    private volatile Limits limits = Limits.none();

    /**
     * Куда сообщается о шагах этого запуска: приёмник отладки.
     * <p>
     * Свойство запуска по той же причине, что метрики, профиль и пределы: сессия
     * отладки — это сеанс целиком, а не область видимости. По умолчанию отладки нет
     * ({@link Debugger#off()}).
     */
    private volatile Debugger debugger = Debugger.off();

    /**
     * Отлаживается ли этот запуск.
     * <p>
     * Плоский {@code boolean}, а не {@code debugger != Debugger.off()} и не метод
     * приёмника: этот флаг читается <b>перед каждой инструкцией скрипта</b> — самое
     * горячее место движка после поиска имени. Чтение поля и сравнение JIT сворачивает
     * вместе с выключенной веткой; вызов через интерфейс он тоже, скорее всего, свернёт,
     * но «скорее всего» в такой точке — не тот ответ, на котором стоит держать
     * обещание «выключённое стоит ноль».
     */
    private volatile boolean debugging;

    /**
     * Собранные формы классов и трейтов. Один на запуск, иначе объявление класса
     * внутри функции давало бы новую форму на каждый вызов, а {@code is} перестал бы
     * узнавать свои же экземпляры.
     */
    private final Linker linker = new Linker();

    /** Классы ошибок и {@code Closeable} этого запуска — снимок прелюдии. */
    private final PreludeTypes exceptions = new PreludeTypes();

    /**
     * Члены, добавленные типам и классам в этом запуске: {@code extend} из скрипта
     * и наборы от приложения.
     * <p>
     * Свойство запуска, а не процесса: основание ({@link BuiltinMembers}) общее
     * и неизменяемое, а надстройка у каждого интерпретатора своя — иначе один скрипт
     * менял бы поведение значений в другом.
     */
    private final MemberTable members = new MemberTable();

    /**
     * Состояние потока: открытые внешние входы и шаги, ещё не влитые в общий счётчик.
     * <p>
     * Массивом на два числа, а не двумя {@code ThreadLocal}: оба правятся на горячем
     * пути, и второй поход в карту потока стоил бы ровно столько же, сколько первый.
     * Боксов здесь тоже нет по той же причине.
     */
    private final ThreadLocal<long[]> state = ThreadLocal.withInitial(() -> new long[2]);

    /**
     * Сделанные шаги — общие на запуск.
     * <p>
     * Общие, потому что «восемь потоков по лимиту каждый» — это не лимит.
     * {@link LongAdder}, а не {@code AtomicLong}: складывают его многие, читают редко
     * (раз в батч), и это ровно тот случай, под который он и сделан.
     */
    private final LongAdder steps = new LongAdder();

    /**
     * Момент, когда время запуска выйдет, в шкале {@link System#nanoTime()};
     * {@code 0} — таймаута нет или отсчёт ещё не начат.
     * <p>
     * Ставится при <b>первом входе в скрипт</b>, а не при создании запуска: сборка
     * движка, чтение файлов и разбор модулей не должны съедать бюджет скрипта.
     */
    private final AtomicLong deadline = new AtomicLong();

    /** Вышло ли время. Ставит сторож или первая же проверка дедлайна. */
    private volatile boolean expired;

    /** Сторож времени или {@code null}, если таймаута нет. */
    private volatile Thread watchdog;

    /**
     * Потоки, находящиеся внутри запуска прямо сейчас.
     * <p>
     * Нужны сторожу: прерывать он вправе именно их — и потоки скрипта, и поток
     * приложения, который вошёл в скрипт и там застрял. {@link RunThreads} знает
     * только про первых.
     */
    private final Set<Thread> inside = ConcurrentHashMap.newKeySet();

    /** Закрыт ли запуск. После закрытия вход в скрипт даёт остановку выполнения. */
    private volatile boolean closed;

    /** Потоки, заведённые скриптом: их же и останавливать при закрытии. */
    private final RunThreads threads = new RunThreads(this);

    /**
     * Пустой запуск: модулей нет вовсе.
     * <p>
     * Не «модули отключены флагом», а «источника не существует»: движок, встроенный
     * в приложение, не читает чужие файлы и не открывает сеть, пока его об этом
     * не попросили набором.
     *
     * @param root корневая область запуска — родитель областей всех его модулей
     */
    Run(Environment root) {
        this.modules = new Modules(new ModuleUnits(ModuleSource.none()), NativeModules.none(),
                Objects.requireNonNull(root, "root"));
    }

    public MemberTable members() {
        return members;
    }

    public Modules modules() {
        return modules;
    }

    public Linker linker() {
        return linker;
    }

    public PreludeTypes exceptions() {
        return exceptions;
    }

    public RunThreads threads() {
        return threads;
    }

    /** Задаёт модули запуска. Зовётся при сборке, до первой инструкции скрипта. */
    void useModules(Modules replacement) {
        this.modules = Objects.requireNonNull(replacement, "modules");
    }

    Metrics metrics() {
        return metrics;
    }

    /** Задаёт приёмник метрик. Зовётся при сборке, до первой инструкции скрипта. */
    void useMetrics(Metrics replacement) {
        this.metrics = Objects.requireNonNull(replacement, "metrics");
    }

    /**
     * Приёмник профиля этого запуска; по умолчанию — выключенный.
     * <p>
     * Публичный, в отличие от {@link #metrics()}: точки съёма живут в тех же местах,
     * что и {@link #checkpoint(Span)}, — в интерпретаторе, в теле функции и в создании
     * экземпляра, — и все они спрашивают приёмник здесь.
     */
    public Profiler profiler() {
        return profiler;
    }

    /** Задаёт приёмник профиля. Зовётся при сборке, до первой инструкции скрипта. */
    void useProfiler(Profiler replacement) {
        this.profiler = Objects.requireNonNull(replacement, "profiler");
    }

    /** Пределы этого запуска. */
    public Limits limits() {
        return limits;
    }

    /** Задаёт пределы. Зовётся при сборке, до первой инструкции скрипта. */
    void useLimits(Limits replacement) {
        this.limits = Objects.requireNonNull(replacement, "limits");
    }

    /**
     * Отлаживается ли запуск. Спрашивается перед каждой инструкцией — отсюда
     * и плоский флаг вместо вопроса приёмнику.
     */
    public boolean debugging() {
        return debugging;
    }

    /**
     * Приёмник отладки этого запуска; по умолчанию — выключенный.
     * <p>
     * Публичный, как и {@link #profiler()}, и по той же причине: точки съёма живут
     * в интерпретаторе и в {@link #checkpoint(Span)}, и спрашивают приёмник здесь.
     */
    public Debugger debugger() {
        return debugger;
    }

    /**
     * Задаёт приёмник отладки.
     * <p>
     * В отличие от метрик и профиля — <b>можно и посреди работы</b>: отладчик
     * подключается к живому запуску (режим {@code attach}), и требовать, чтобы игра
     * перезапускалась ради точки останова, значило бы не иметь этого режима вовсе.
     * Порядок записи важен: сначала приёмник, потом флаг, — иначе поток, увидевший
     * флаг, успел бы спросить ещё не поставленный приёмник.
     */
    void useDebugger(Debugger replacement) {
        Debugger known = Objects.requireNonNull(replacement, "debugger");
        this.debugger = known;
        this.debugging = known != Debugger.off();
    }

    /** Сколько шагов скрипт уже сделал: влитые в общий счётчик, без остатков в потоках. */
    public long steps() {
        return steps.sum();
    }

    /**
     * Открывает внешний вход в скрипт: считает вход этого потока.
     * <p>
     * Зовётся там, где управление приходит из-за границы движка, — из
     * {@link Interpreter#run} и из {@link UserFunction#call}, когда поток ещё
     * не внутри этого запуска. Изнутри скрипта не зовётся вовсе: там уже вошли,
     * и платить за это второй раз незачем.
     * <p>
     * Здесь же начинается отсчёт времени: первый вход ставит дедлайн и заводит сторожа.
     *
     * @param span место, которому принадлежит вопрос, — для сообщения об отказе
     */
    void enter(Span span) {
        if (closed) {
            // Не ошибка скрипта, а остановка: ресурсы запуска уже закрыты, и продолжать
            // выполнение по ним нечестно — обработчик такое ловить не должен.
            throw FatalError.runClosed(span);
        }
        long[] slot = state.get();
        if (slot[ENTRIES] >= limits.maxEntries()) {
            throw FatalError.tooDeep(span, limits.maxEntries(),
                    "Слишком длинная цепочка вызовов между скриптом "
                    + "и приложением: проверьте, не зовёт ли обработчик сам себя");
        }
        if (slot[ENTRIES]++ == 0) {
            inside.add(Thread.currentThread());
        }
        armTimeout();
    }

    /** Закрывает внешний вход. Зовётся только в {@code finally} к {@link #enter}. */
    void leave() {
        long[] slot = state.get();
        flushSteps(slot);
        if (--slot[ENTRIES] == 0) {
            inside.remove(Thread.currentThread());
            // Не держим запись в карте потока: пул живёт дольше запуска, и оставленный
            // счётчик тянул бы за собой ссылку на закончившийся сеанс.
            state.remove();
            if (expired) {
                // Флаг прерывания поставил сторож этого запуска — наружу он не уходит.
                // Поток приложения, вошедший в скрипт, прерывается законно: он внутри
                // скрипта. Но унести флаг обратно и получить InterruptedException
                // где-нибудь в своём коде приложение не должно.
                Thread.interrupted();
            }
        }
    }

    /**
     * Работает ли текущий поток внутри этого запуска прямо сейчас.
     * <p>
     * Спрашивается у собственного счётчика, а не у вызывающего, и это принципиально.
     * Напрашивается проверить контекст — «пришёл ли вызов от {@code ExecutionContext}
     * этого запуска», — но она отвечает не на тот вопрос: приложение вправе держать
     * контекст запуска у себя и звать функцию с ним из любого потока. Контекст сказал бы
     * «свой», вход не был бы засчитан, и цепочка «скрипт → приложение → скрипт»
     * перестала бы упираться в предел входов.
     */
    boolean insideCurrentThread() {
        return state.get()[ENTRIES] > 0;
    }

    /**
     * Точка проверки: не пора ли остановить выполнение.
     * <p>
     * Одна на все причины — прерывание снаружи, исчерпание шагов, истечение времени, —
     * и зовётся она оттуда, где скрипт делает шаг: из циклов ({@link Interpreter}),
     * из тела функции ({@link UserFunction}) и из создания экземпляра
     * ({@link WdlClass}). Своего обхода дерева она не заводит и заводить не должна.
     * <p>
     * <b>Выключенные лимиты стоят ровно столько, сколько стоила прежняя проверка
     * флага</b>: чтение поля и сравнение. Ни {@code nanoTime}, ни общего счётчика
     * на этом пути нет.
     *
     * @param span место, которому принадлежит шаг, — оно попадёт в сообщение
     */
    void checkpoint(Span span) {
        if (Thread.currentThread().isInterrupted()) {
            throw stopped(span);
        }
        // Вторая точка шага для отладчика — там, где шаг есть, а инструкции нет:
        // итерация цикла, вызов, создание экземпляра. Без неё скрипт из функций-выражений
        // (тело через '=>', без единой инструкции) не встал бы по паузе никогда.
        // Раньше пределов намеренно: просьбу встать выполняем прежде, чем сообщать
        // об исчерпании шагов.
        if (debugging) {
            debugger.poll(span);
        }
        Limits current = limits;
        if (!current.counting()) {
            return;
        }
        long[] slot = state.get();
        if (++slot[STEPS] < current.stepBatch()) {
            return;
        }
        long batch = slot[STEPS];
        slot[STEPS] = 0;
        steps.add(batch);
        long max = current.maxSteps();
        if (max > 0 && steps.sum() >= max) {
            throw FatalError.stepsExhausted(span, max);
        }
        long at = deadline.get();
        if (at != 0 && System.nanoTime() - at >= 0) {
            expired = true;
            throw FatalError.timedOut(span, current.timeout());
        }
    }

    /**
     * Не слишком ли велико одно выделение байтов ({@link Limits#maxBufferBytes()}).
     * <p>
     * <b>Отдельно от {@link #checkpoint(Span)}, и это то же разделение, что у квоты
     * потоков:</b> шаги и время отвечают на вопрос «сколько работы сделано»,
     * а выделение — на «сколько ресурса выдано за раз». Счётчик шагов растущий
     * по элементу массив останавливает сам, а {@code bin.zeros(1 << 40)} тратит
     * ровно один шаг, и увидеть его можно только здесь — там, где ресурс выдаётся.
     * <p>
     * <b>Обычная ошибка скрипта, а не остановка запуска</b> — снова как у квоты
     * потоков: «это выделение слишком велико» значит «эта операция не удалась»,
     * и поймать такое обработчиком совершенно законно. Остановкой ({@link FatalError})
     * отвечают исчерпанные шаги и время, потому что после них продолжать нечего.
     * <p>
     * Живой объём не считается намеренно: освобождение отдаёт сборщик мусора,
     * и счётчик разошёлся бы с реальностью — см. {@link Limits}.
     *
     * @param bytes сколько байтов просят выделить
     * @param what  что именно выделяют — попадёт в сообщение: {@code "bin.zeros()"}
     */
    void allocating(long bytes, String what, Span span) {
        Limits current = limits;
        if (!current.limitsBuffers()) {
            return;
        }
        long max = current.maxBufferBytes();
        if (bytes > max) {
            throw new WdlRuntimeError(ErrorKind.VALUE, span, what + ": запрошено "
                    + bytes + " байт, а одним выделением разрешено " + max
                    + ". Читайте данные кусками — например, 'streams.chunks(path, 65536)'");
        }
    }

    /**
     * Чем именно остановлено выполнение прерванного потока.
     * <p>
     * Разница видна и в сообщении, и в трассировке: «прервано снаружи» и «вышло время»
     * — разные события, и хозяину запуска они говорят разное. Прерывание от сторожа
     * узнаётся по {@link #expired}: другого способа отличить свой {@code interrupt}
     * от чужого у потока нет.
     */
    private FatalError stopped(Span span) {
        return expired ? FatalError.timedOut(span, limits.timeout()) : FatalError.interrupted(span);
    }

    /**
     * Называет остановку своим именем на границе запуска.
     * <p>
     * Библиотека, вышедшая из блокирующего вызова по {@code interrupt}, честно отвечает
     * «выполнение прервано» — про сторожа она не знает и знать не должна. Но хозяину
     * запуска, поставившему таймаут, нужен ответ про <b>время</b>: «прервано» он читает
     * как «кто-то нажал стоп», а никто не нажимал.
     */
    FatalError explain(FatalError stop) {
        if (!expired || !stop.isInterruption()) {
            return stop;
        }
        return FatalError.timedOut(stop.span(), limits.timeout()).inSource(stop.source());
    }

    /** Вливает остаток шагов потока в общий счётчик. */
    private void flushSteps(long[] slot) {
        if (slot[STEPS] > 0) {
            steps.add(slot[STEPS]);
            slot[STEPS] = 0;
        }
    }

    /**
     * Начинает отсчёт времени, если таймаут задан и отсчёт ещё не начат.
     * <p>
     * Гонку двух первых входов разрешает {@code compareAndSet}: сторож один на запуск,
     * и завести его вправе только тот, кто поставил дедлайн.
     */
    private void armTimeout() {
        long nanos = limits.timeoutNanos();
        if (nanos == 0 || deadline.get() != 0) {
            return;
        }
        long at = System.nanoTime() + nanos;
        // Ноль в этой шкале — «отсчёт не начат», поэтому настоящий момент нулём быть
        // не может: сдвиг на наносекунду дешевле второго поля-признака.
        if (at == 0) {
            at = 1;
        }
        if (deadline.compareAndSet(0, at)) {
            startWatchdog();
        }
    }

    /**
     * Отодвигает срок на время, которое запуск простоял на паузе отладчика.
     * <p>
     * Без этого сессия отладки живёт ровно столько, сколько разрешено скрипту:
     * под {@code Stdlib.SAFE} это тридцать секунд, после которых человек, думающий
     * над панелью переменных, получает «вышло время выполнения» — сообщение,
     * не имеющее к делу никакого отношения. Часы запуска меряют работу скрипта,
     * а на паузе он не работает.
     * <p>
     * Зовёт сессия, и <b>один раз на паузу целиком</b>, а не на каждый вставший поток:
     * восемь потоков, простоявших одну минуту, простояли минуту, а не восемь.
     * <p>
     * Таймаута нет — не делается ничего: сдвигать нечего, и заводить часы задним
     * числом было бы хуже, чем не иметь их вовсе.
     *
     * @param nanos сколько длилась пауза
     */
    public void extendDeadline(long nanos) {
        if (nanos <= 0) {
            return;
        }
        deadline.updateAndGet(at -> at == 0 ? 0 : at + nanos);
        // Сторожа будить не надо: он проснётся по старому сроку, перечитает поле
        // и увидит, что время ещё не вышло, — см. watch().
    }

    /**
     * Заводит сторожа времени.
     * <p>
     * <b>Обычным {@code new Thread}, а не через {@link RunThreads},</b> и это
     * единственное такое место. Правило «поток только через реестр» защищает от того,
     * чтобы поток, переживший закрытие, позвал функцию скрипта по закрытым модулям;
     * сторож же кода скрипта не выполняет вовсе — он умеет только {@code interrupt}.
     * А попади он в реестр, он считался бы в квоту потоков скрипта и прерывал бы
     * сам себя.
     */
    private void startWatchdog() {
        // Стек маленький намеренно: тело сторожа — цикл ожидания, кадров ему не надо.
        Thread guard = new Thread(null, this::watch, "wdl-timeout", 64L * 1024);
        guard.setDaemon(true);
        watchdog = guard;
        guard.start();
    }

    /**
     * Тело сторожа: дождаться срока и прервать тех, кто внутри.
     * <p>
     * Срок перечитывается на каждом круге, а не запоминается при старте: пауза
     * отладчика отодвигает его ({@link #extendDeadline(long)}), и сторож, спящий
     * до старого срока, обязан проснуться, увидеть новый и досыпать.
     */
    private void watch() {
        try {
            while (true) {
                long left = deadline.get() - System.nanoTime();
                if (left <= 0) {
                    break;
                }
                // Миллисекунды с округлением вверх: проснуться на наносекунду раньше
                // значило бы крутиться в цикле до самого срока.
                Thread.sleep(left / 1_000_000L + 1);
            }
        } catch (InterruptedException stop) {
            // Запуск закрылся раньше срока — сторожить больше нечего.
            return;
        }
        if (closed) {
            return;
        }
        expired = true;
        interruptInside();
    }

    /** Прерывает всех, кто сейчас внутри запуска, — и потоки скрипта, и вошедших. */
    private void interruptInside() {
        for (Thread thread : inside) {
            thread.interrupt();
        }
        threads.interruptAll();
    }

    /**
     * Помечает запуск закрытым: дальше вход в скрипт даёт остановку выполнения.
     * <p>
     * Нужно затем, что живые потоки скрипта переживают {@code close()} ровно столько,
     * сколько нужно им, чтобы заметить прерывание, — и позвать функцию по уже закрытым
     * модулям они не должны. Внятная остановка тут честнее, чем работа по закрытому
     * соединению.
     * <p>
     * Здесь же снимается сторож времени: сеанса больше нет, сторожить нечего.
     */
    void close() {
        closed = true;
        Thread guard = watchdog;
        if (guard != null) {
            watchdog = null;
            guard.interrupt();
        }
    }

    /** Закрыт ли запуск. */
    boolean isClosed() {
        return closed;
    }
}
