 ...
 * Если бы интерпретатор принимал {@link Environment} напрямую, каждое такое добавление
 * означало бы правку подписи всех методов посетителя и всех его реализаций.
 * <p>
 * Счётчики лимитов сюда, наоборот, <b>не попали</b>: контекст копируется на каждую
 * область видимости, и счётчик в нём обнулялся бы на входе в любой блок. Их место —
 * {@link Run}, и туда же смотрит {@link #withLimits(Limits)}.
 * <p>
...
public final class ExecutionContext implements CallContext {
    ...
    /**
     * Тот же контекст, но с пределами выполнения: шагами, временем, квотой потоков.
     * <p>
     * По умолчанию пределов нет — движок не платит за то, о чём его не просили, ровно
     * как не печатает никуда и не считает время стадий. Ставятся они при сборке,
     * до первой инструкции скрипта: «сколько шагов осталось» — свойство запуска,
     * и менять предел посреди работы значило бы, что ответ зависит от момента вопроса.
     * <p>
     * Отсчёт времени начинается не здесь, а на первом входе в скрипт: чтение файлов
     * и разбор модулей не должны съедать бюджет скрипта. См. {@link Limits}.
     */
    public ExecutionContext withLimits(Limits limits) {
        run.useLimits(limits);
        return this;
    }
