...
final class ThreadState {
    ...
    /**
     * Идёт ли сейчас раскрутка ошибки, о которой потоку уже сообщили.
     * <p>
     * Одна ошибка проходит наружу через всю цепочку вложенных инструкций и о себе
     * сообщает на каждой. Останавливаться надо на первой — самой глубокой, где она
     * и случилась. Отличить «та же ошибка» от новой по тождеству объекта нельзя:
     * граница функции пересоздаёт ошибку, дописывая ей файл ({@code inSource}),
     * и наружу из {@code f()} летит уже другой объект с тем же смыслом.
     * <p>
     * Зато точно известно другое: <b>между двумя сообщениями об одной ошибке поток
     * не делает ни шага</b>. Поэтому признак снимается в {@code at}: пошёл дальше —
     * значит, раскрутка кончилась (ошибку поймал {@code catch}), и следующая ошибка
     * будет новой.
     */
    volatile boolean unwinding;
