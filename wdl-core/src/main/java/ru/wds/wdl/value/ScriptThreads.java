...
public interface ScriptThreads {
    ...
    /**
     * То же, но поток заявляет, <b>чем его будить</b>.
     * <p>
     * Прерывание доходит не до всякого потока. Стоящий в {@code accept()},
     * {@code read()} или {@code readLine()} не выходит из них по {@code interrupt}
     * вовсе: его будит только конец ввода или закрытие сокета. Такой поток, заведённый
     * обычным {@link #start}, честно прерывается — и продолжает висеть, а закрытие
     * запуска ждёт его весь свой срок и называет в логе. Ровно это и делали слушатели
     * {@code sys.net.socket}: каждый скрипт с сокетом заканчивался пятисекундной
     * паузой и строкой про незавершившиеся потоки.
     * <p>
     * Будильник — дело того, кто завёл поток: только он знает, чем этот поток занят.
     * Зовётся он <b>перед</b> прерыванием и не должен ни ждать, ни бросать: его дело —
     * один короткий вызов, после которого блокирующая операция вернёт управление
     * ({@code socket.shutdownInput()}, {@code server.close()}).
     *
     * @param waker что позвать перед прерыванием; {@code null} — будить нечем
     */
    default Thread start(String name, Runnable body, Runnable waker) {
        // Умолчание игнорирует будильник намеренно: реестра нет — значит и останавливать
        // эти потоки некому, будить некого и незачем (см. unmanaged()).
