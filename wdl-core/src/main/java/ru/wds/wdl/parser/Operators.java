...
final class Operators {
    ...
    /**
     * Лямбда {@code p => p * 2} — слабее всего, что есть в выражении.
     * <p>
     * Слабее нарочно: тело обязано забирать всё, что написано справа, иначе
     * {@code x => x > 0 ? 1 : -1} оторвало бы тернарник от тела. В таблице лямбды нет
     * (правая часть у неё не операнд, а тело), но число ей нужно — {@code expression}
     * сверяет его с порогом ровно так же, как у бинарного оператора.
     * <p>
     * Тело разбирается с порогом, <b>равным</b> этому числу, а не на единицу большим:
     * отсюда правая ассоциативность, то есть каррирование {@code a => b => a + b}.
     */
