...
public enum BinaryOp {
     ...
     * <p>
     * <b>Спрашивает про {@code null}, а не про ложность</b> — этим и отличается
     * от {@link #OR}, который в этом языке тоже отдаёт сам операнд, а не признак.
     * Ложны здесь двое — {@code null} и {@code false}, — а пусто только одно,
     * и вся разница поэтому в {@code false}: {@code verbose ?? true} оставляет
     * выключённое выключенным, {@code verbose || true} его включает. Оба нужны,
     * потому что вопросы разные: «значение не задано» и «значение не годится».
     * <p>
     * Ноль, пустая строка и пустой массив здесь истинны (см. {@code Value.isTruthy}),
     * поэтому в отличие от языков, где ложен и ноль, {@code ??} не спасает от подмены
     * нуля — от неё спасает {@code ||}, которому нечего подменять.
     * <p>
     * Ленивая, как {@link #AND} и {@link #OR}: правая часть вычисляется, только если
     * слева пусто. И <b>неперегружаемая</b> ({@code member() == null}) — перегрузить
     * «что считать пустым» значит разрешить объекту соврать про собственное
     * отсутствие, после чего ни одна проверка на {@code null} ничего не гарантирует.
     */
    COALESCE("??", null, true),
