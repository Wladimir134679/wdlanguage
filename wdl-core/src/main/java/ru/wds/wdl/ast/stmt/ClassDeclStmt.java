...
public record ClassDeclStmt(
     ...
     * класс. Простое имя ({@code Shape}) остаётся частным случаем, и важным: только
     * по нему видно, кто от кого зависит, поэтому только оно и позволяет
     * {@link ru.wds.wdl.resolve.Resolver резолверу} расставить объявления так, чтобы
     * родитель шёл раньше потомка. Класс, чей родитель записан выражением, появляется
     * строго на своей строке — как и класс, наследующий от {@code m.Shape}: вычислить
     * выражение до первой инструкции скрипта значило бы выполнить его в момент,
     * когда скрипта ещё не было.
     * <p>
