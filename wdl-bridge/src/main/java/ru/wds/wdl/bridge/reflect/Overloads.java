...
public final class Overloads {
     ...
     */
    /**
     * Сколько аргументов принимает имя, за которым стоит набор перегрузок.
     * <p>
     * Диапазон по всем кандидатам: проверка числа аргументов до вызова должна
     * пропустить любую подходящую перегрузку, а выбирать между ними — дело
     * {@link #select}.
     */
    public static Arity arityOf(List<JavaExecutable> candidates) {
        if (candidates.isEmpty()) {
            // Звать нечего — и сказать об этом должен сам вызов, а не проверка числа
            // аргументов перед ним: «принимает ровно 0, а передано 1» уводит не туда.
            return Arity.any();
        }
        int min = Integer.MAX_VALUE;
        int max = 0;
        for (JavaExecutable candidate : candidates) {
            min = Math.min(min, candidate.minimum());
            max = Math.max(max, candidate.varargs() ? Integer.MAX_VALUE : candidate.count());
        }
