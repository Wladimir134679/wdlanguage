...
| `wdl-game` | WDGame: маленький игровой движок на Swing/Graphics2D и модуль `game` к нему — площадка для проверки встраивания | `core`, `bridge`, `api` |
...
* [Игровой движок](docs/game.md) — модуль `game`: мир и спрайты, порядок кадра,
  жизненный цикл, ввод и холст; зачем движок живёт в этом репозитории и что именно
  он проверяет.
...
а не язык.
Отдельным модулем сборки живёт игровой движок `wdl-game` (WDGame): окно на Swing,
сцена со спрайтами, цикл кадров и холст с попиксельным рисованием — всё это отдано
скрипту модулем `game`. В стандартную библиотеку он не входит и не войдёт: это
площадка, на которой встраивание проверяется самым требовательным приложением
из возможных — игра зовёт скрипт шестьдесят раз в секунду и ждёт ответа
до следующего кадра. Пинг-понг целиком на wdl лежит в
[examples/game/pong.wdl](examples/game/pong.wdl), запускается он движком:
`./gradlew :wdl-game:run --args="examples/game/pong.wdl"` — см.
