...
* [Отладчик](docs/debugging.md) — остановка на строке, кадры со своими переменными,
  ...
  отладчик не может и сколько стоит выключенный.
...
в серии 8–10 %) — см. [docs/debugging.md](docs/debugging.md). Транспорта пока нет:
отладка из редактора идёт по протоколу DAP, модуль `wdl-dap` в работе, и до него
у консоли нет флага `--debug-port`.
