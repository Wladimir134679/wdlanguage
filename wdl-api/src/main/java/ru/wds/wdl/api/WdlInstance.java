...
public final class WdlInstance implements AutoCloseable {
     ...
     *   <li><b>Отладка</b> — после модулей и по той же причине, по которой вход
     *       закрывается последним. Раз модуль вправе звать скрипт в своём закрытии,
     *       то и точка останова в этом коде обязана срабатывать: у скрипта с
     *       интерфейсом весь сеанс отладки обработчиков целиком лежит внутри пункта 2,
     *       и сессия, отпущенная первой строкой, не остановила бы ни одного нажатия.
     *       Ждать человека, отошедшего от отладчика, это не заставляет:
     *       {@code stopThreads()} потоки скрипта <b>прерывает</b>, а прерванный поток
     *       выходит из парковки сам.</li>
    ...
    public void close() {
        ...
        try {
            ...
            closeRootLibraries();
            // Отладка отпускается здесь, а не первой строкой: пункт 2 — это время,
            // когда модуль зовёт скрипт, и точка останова в обработчике кнопки обязана
            // сработать так же, как в теле файла. Ждать человека это не заставляет:
            // stopThreads() прерывает потоки скрипта, а прерванный из парковки выходит.
            releaseDebugger();
