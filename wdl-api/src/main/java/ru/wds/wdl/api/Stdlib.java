...
public enum Stdlib {
     ...
     * {@code sys.meta}, {@code sys.time} и {@code sys.thread}.
     * <p>
     ...
     * <p>
     * <b>Набор — это модули плюс пределы.</b> {@code sys.thread} мира не трогает,
     * но поток — ресурс процесса, и до появления
     * {@linkplain ru.wds.wdl.runtime.Limits лимитов выполнения} модуля здесь
     * не было: {@code for (;;) th.spawn(...)} клал не скрипт, а хозяина. Теперь
     * {@code WdlEngine.Builder.stdlib(SAFE)} ставит вместе с модулями
     * и {@code Limits.safeDefaults()} — шаги, время и квоту потоков. Сняв их явным
     * {@code limits(Limits.none())}, приложение снимает и обещание этого набора.
     */
    SAFE,
    ...
    Map<String, Supplier<Library>> modules() {
        ...
        switch (this) {
            ...
            case SAFE -> {
                ...
                // Потоки — ресурс процесса, и держит их здесь не безобидность модуля,
                // а квота запуска: см. javadoc набора.
