 ...
 * файла, и это не заплатка: ждать честнее, чем изображать асинхронность там, где
 * её нет. Ждать при этом можно не мешая остальным — запрос, отданный в {@code
 * th.spawn} или в пул, держит только свой поток ({@code docs/threads.md}).
 * <p>
 ...
 * <p>
 * <b>Тело — строка или байты.</b> Строка уходит в UTF-8, чего хватает JSON, формам
 * и тексту; {@code bytes} уходят как есть, байт в байт. Ответ лежит двумя полями
 * по той же причине — {@code r.bytes} и {@code r.body}, и почему это не дублирование,
 * написано у класса ответа.
 */
