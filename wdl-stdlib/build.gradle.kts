...
dependencies {
    ...
    // Только тестам: каталог снимается с этих же модулей, и проверять снимок надо
    // там, где модули объявлены. Сама библиотека про инструменты не знает —
    // направление зависимостей от этого не меняется.
    testImplementation(project(":wdl-tools"))
}
